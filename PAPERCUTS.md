...
## 2026-08-25 â€” Devin's review never finishes on a large PR
...
- **Cut:** On BloomDesktop PR #8229 (74 files, ~15,000 insertions) the jobs API reports the job
  ...
  1d3f89e69, 726926841) â€” 23 jobs on the PR in total, not one producing findings. Waited 35 minutes
...
- **Idea:** Have `devin-review` treat "job completed but lifeguard still pending past N minutes" as
  ...
  sub-agent review when Devin yields nothing â€” a Fable-model reviewer stood in here and did the
  job, and may be the better default above some diff size.
- **Context:** BL-16719, PR #8229, 2026-08-25. Devin's own review page shows the same nothing.

## 2026-08-24 â€” A CSS transition never runs in a background tab, so hover looks broken
...
- **seen again 2026-08-24:** two more things a background tab will not do.
  ...
  (`Object.defineProperty(navigator, 'clipboard', â€¦)` capturing the text) so the rest of the
...
## 2026-08-24 â€” Two agents on one Chrome: screenshots die, javascript_tool lives
...
## 2026-08-23 â€” Devin repeats a whole finding set on the next commit, at lines that have moved
...
## 2026-08-23 â€” Headless Chrome writes `--screenshot` beside chrome.exe, and calls it access denied
...
## 2026-08-14 â€” Preflight's review sub-agent edited the live tree it was reviewing
...
- **Cut:** The light local review in `preflight` Phase 1 is dispatched as a general-purpose
  ...
  point by editing the code under review â€” replacing `lock (GetFileLock(fullPath))` with
  `if (true) // TEMP-REVIEW-NO-LOCK` â€” while the session's full C# suite was running against
...
## 2026-08-12 â€” In multi-agent sessions, Write silently clobbers a sibling agent's new file
...
## 2026-08-06 â€” The Remove-Item safety hook mis-parses a quoted path containing a space
...
## 2026-07-28 â€” Markdown links in Claude Code replies aren't clickable in Orca's terminal

- **Cut:** `[BL-16618](https://â€¦)` in a chat reply renders coloured + underlined but does
  nothing on click, and the URL is hidden inside the markup so it can't be copied either â€”
  ...
  nothing, so hyperlinks are suppressed. Orca's own terminal is fine â€” it bundles xterm.js with
...
## 2026-07-23 â€” nx test runner hangs with no output; direct vitest is instant

- **Cut:** Running a package's tests through nx (`npm run â€¦ testonce` â†’ `nx vite:test`, or
  `nx run-many --target test`) produced **zero output for 7+ minutes** while the node process
  stayed alive â€” indistinguishable from a hang. The identical suite run directly with
...
- **Idea:** Figure out whether it's the nx daemon on this machine or output buffering in nx's
  task runner â€” try `nx reset`, `NX_DAEMON=false`, or check the daemon logs. If it's chronic,
...
- **Context:** Preflight run on EthnoLib (worktree `C:\dev\EthnoLib.worktrees\next`, PR #150),
  ...
  vitest.config.ts`.
- seen again: 2026-08-12 (EthnoLib CharacterVariants branch, Hatton's machine) â€” `nx run-many
...
## 2026-07-18 â€” One shared local Supabase stack, many parallel agents
...
- **Cut:** Parallel agents (and sessions) all share the single local Supabase stack for
  ...
  in-place data scrub mid-task (books went 699â†’0â†’699 under it); the second agent only
...
## 2026-07-18 â€” Background subagents go idle without delivering their final report

- **Cut:** Claude Code background agents (Agent tool) frequently finish and emit only an
  `idle_notification` â€” the final report never arrives until the coordinator sends a
...
- seen again: 2026-07-18 (decisions round) â€” opus agent idled twice without its report even
...
## 2026-07-18 â€” Orca worktree rm hangs on zombie PTYs; workaround is sending "exit"
...
## 2026-07-17 â€” Machine-wide SUPABASE_* env vars silently redirect tools to a cloud project
...
- **Cut:** John's machine has global `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` env vars
  ...
  conventional `SUPABASE_*` names targets that project instead of the local stack â€” the new
...
## 2026-07-14 â€” plannotator-last shows a blank page when reviewing plans
...
## 2026-07-14 â€” xlf-strings skill overstates the risk of editing a translated `<source>`
...
## 2026-07-29 â€” The Read tool renders `//` as `\` in some source files, so comments look like syntax errors
- **Cut:** Reading `packages/lib/src/4-generate-html/html-generator.ts` with the Read tool, several
  line comments came back with the `//` replaced by a single backslash â€” e.g.
  `const MARGIN_PX = 12 * MM_TO_PX; \ --page-margin: 12mm` and
  `return i < count - 1 ? 1 \ 2 ** (i + 1) : 1 / 2 ** i;`. Both are valid `//` in the file
  (confirmed with `grep | cat -A`). The mangling is *inconsistent within one Read* â€” some `//`
  survive, some don't â€” and it lands on real code (`1 \ 2 **` reads as a broken expression), so I
...
- **Idea:** Worth reporting upstream as a Read/display bug. Meanwhile: if a Read shows a lone `\`
  where an operator or comment marker belongs, don't trust it â€” confirm with `grep -n â€¦ | cat -A`
...
## 2026-07-29 â€” The YouTrack Bot account cannot set Assignee at all, and no skill says so
- **Cut:** Asked to file a card "assigned to me", I created it fine (Type, Kanban Board, State all
  ...
  Bot has no read access to the user directory, so it cannot *resolve* any assignee. Evidence â€”
  `GET /api/users` returns only `Bot` itself; the Assignee values on existing issues all come back
  as `anonymized-6104`, `anonymized-8057`, â€¦; `commands/assist` on `"Assignee "` offers no user
...
- **Idea:** `youtrack-create-issue` should say up front that **Assignee is not settable by the Bot**
  â€” set Type/board/State, then hand the user the link and ask them to assign (one click), rather
  than letting an agent discover it by trial and error. Same for `youtrack-api` Â§fields. If we do
...
## 2026-08-04 â€” `node -e` from the Bash tool: output can vanish, and module resolution follows the script, not cwd
- **Cut:** Wanted a quick "does this LESS still compile" check. `node -e "â€¦"` with a multi-line
  ...
  MODULE_NOT_FOUND â€” node resolves from the *script's* directory, and the scratchpad has no
...
- **Idea:** For any throwaway node script that needs the project's deps, write the `.cjs` **inside
  the package directory** (`src/BloomBrowserUI/`), run it, and delete it â€” don't use the scratchpad,
...
## 2026-08-06 â€” Chrome MCP `computer` screenshots dead in this session; `chrome-devtools-cli` covered for it
- **Cut:** Every `mcp__claude-in-chrome__computer` action with `action: screenshot` failed with
  "Script injection timed out after 5000ms â€” the page is busy or mid-navigation", including on a
...
- **Idea:** When `computer screenshot` times out, don't debug the page â€” first sanity-check the tool
  ...
  profile, so localStorage from the extension-driven tab is not there â€” seed it with
...
## 2026-08-13 â€” The Edit tool cannot touch a line containing a zero-width character
- **Cut:** ESLint's `no-irregular-whitespace` flagged a literal U+200B in a spec assertion
  (`expect(entries).not.toContain("â€‹")` written with the real character). Every attempt to
  ...
  call comes back "No changes to make: old_string and new_string are exactly the same" â€” even
...
- **Idea:** Don't try to edit a line whose content includes an invisible or zero-width character.
  Patch it from the shell against the code point instead â€” in PowerShell:
  `$t=[IO.File]::ReadAllText($p); $t=$t.Replace("x$([char]0x200B)x", 'escaped'); [IO.File]::WriteAllText($p,$t)`
  â€” then grep to confirm. Better still, write such characters as `\uXXXX` escapes in the first
...
## 2026-08-13 â€” `npm -C <pkg> exec -- vite` starts vite with the *repo root* as cwd
- **Cut:** To run a package's dev server on a fixed port I used
  ...
  monorepo root as a static site â€” where there is no `index.html`. The banner gives no hint: it
  looks exactly like a healthy server, and the 404 reads as a routing or build problem in the app.
- **Idea:** Point vite at the config by absolute path instead â€” `npx vite --config
  D:/<repo>/<pkg>/vite.config.ts --port <n> --strictPort` â€” which works because these configs set
...
## 2026-08-13 â€” A backgrounded PowerShell command looks hung when its output is piped to `Select-Object -Last`
- **Cut:** I ran `npm run testonce 2>&1 | Select-Object -Last 30` to keep a noisy nx/vitest run
  ...
  completely empty for another five minutes. Everything pointed at a hang â€” an nx daemon waiting on
  stdin, a prompt nobody could answer â€” and I killed it and started diagnosing the wrong problem.
...
## 2026-08-16 â€” Chrome screenshots time out on a busy page, and `find` goes with them
- **Cut:** On the font-chooser demo, every `mcp__claude-in-chrome__computer` screenshot failed with
  ...
  waited 45000ms for document_idle)" â€” for ten minutes, on a page that was rendering perfectly and
...
- **Idea:** `javascript_tool` does not wait for idle and kept working throughout. On a page with
  ...
  For verifying *rendering* â€” the thing a screenshot is actually for â€” `canvas.measureText` with
...
## 2026-08-17 â€” Spawning `npx` from a Node script on Windows: both obvious ways fail
- **Cut:** A skill script needed to run `npx supabase db query ... "<sql>"`. `execFileSync("npx",
  ...
  back as garbage flags and the CLI answered by printing its own help â€” which reads as "you called
...
- **Idea:** Keep `shell: true` and make sure no argument needs quoting: write SQL (or any
  ...
  double quotes yourself. Same shape applies to any `.cmd`/`.ps1` shim â€” npm, pnpm, vercel, tsc.
- **Context:** building the `feedback-from-font-chooser` skill, which dumps the font chooser demo's
  Supabase feedback rows verbatim.

## 2026-08-17 â€” Previewing a locally generated HTML file: neither obvious route works
- **Cut:** I generated a static dashboard page and wanted to look at it. The Chrome extension
  ...
  **Bash tool exits instantly with code 0 and no output** â€” the sandbox blocks listening sockets,
...
## 2026-08-18 â€” `py -m json.tool` makes clean UTF-8 look double-encoded
- **Cut:** Inspecting Supabase rows with `curl ... | py -m json.tool` printed
  `"exemplars [a {ÃƒÂ¢} b ...]"`. `ÃƒÂ¢` is `ÃƒÂ¢`, the textbook signature of UTF-8
  read as Latin-1, so it reads as real mojibake in the database â€” and I went off and wrote a scan
...
- **Also:** Python's `urllib` got a bare 403 from `server.bloomlibrary.org/parse` for a request
  node's `fetch` answered 200 (same URL, same app-id header) â€” probably the missing User-Agent.
  Use node for Parse API probing.
- **Context:** EthnoLib `supporting-data`, checking imported SLDR alphabets for corruption.

## 2026-08-18 â€” ESM import of a repo module by Windows absolute path fails
...
- **Idea:** Import by `file:///D:/repo/tools/lib/x.mjs`, or build it with
  `pathToFileURL(path).href` from `node:url`. Relative specifiers (`./lib/x.mjs`) are also fine â€”
...
## 2026-08-19 â€” A backslash escape inside a Bash heredoc reaches the file as a real newline

- **Cut:** Patching a JS file through `py - <<'PY'` with a Python string holding `\\n` â€” the
  normal way to write a literal `\n` into generated source â€” produced an actual line break in the
...
- **Idea:** Never put a backslash escape in a string that goes through a shell heredoc. Build the
  ...
  scripts â€” it sits right next to the existing "use the Write tool for scratch scripts" cut.
- **Context:** EthnoLib `supporting-data`, adding a `--compare-sldr` mode to
  `tools/importBloomBooks.mjs`.

## 2026-08-19 â€” Chrome extension screenshots time out, so a page cannot be looked at

- **Cut:** Every `mcp__claude-in-chrome__computer` screenshot in a session failed with `Script
  injection timed out after 5000ms â€” the page is busy or mid-navigation`, on four different pages
...
- **Idea:** Two failures on two *different* pages means the extension, not the page â€” stop and say
  ...
  `vite preview` binds `localhost` only, not `127.0.0.1` â€” the other spelling gives an error page).
...
- **Context:** EthnoLib `supporting-data`, building an HTML report and a dashboard tab whose whole
  point was a colour-blend diagram â€” the one thing that most needed a visual check.
...
- **Idea:** Concurrent agents editing one tree need either a worktree each or non-overlapping file
  sets, and the dispatcher is the only party that can see the overlap â€” the clusters here were
...
## 2026-08-23 â€” Preflight has no path for a solo repo whose work is all on master
...
- **Idea:** The shape that worked is worth naming in `preflight`: a long-lived marker branch
  ...
  each review and close the PR unmerged. It also splits a large backlog cleanly â€” four slices
...
## 2026-08-23 â€” Two report files in dev-process-artifacts differ only in letter case
...
## 2026-08-24 â€” claude-in-chrome screenshots fail on a page a fresh build was just loaded into

- **Cut:** `mcp__claude-in-chrome__computer{action:"screenshot"}` and `read_page` failed on every
  localhost page in one session with "Script injection timed out after 5000ms â€” the page is busy or
