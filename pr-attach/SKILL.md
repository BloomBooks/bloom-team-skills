...
Uploading is silent and safe — it publishes nothing on its own; the asset just sits
unposted until referenced. This skill only produces the embed markdown. Whether and when
to actually push that markdown into a PR body or comment — and under whose GitHub identity
— is a decision the calling workflow still needs to make deliberately, same as any other
content posted under a user's name.
