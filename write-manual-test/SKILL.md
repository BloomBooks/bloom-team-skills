...
4. If the feature is behind a subscription tier, learn what a person **without** that
 subscription can do with books that used it, and write those rules down too. The
 `add-test-ideas` skill ("Subscription / tier behavior") lists exactly what a derivative made
 below the tier must still allow and what must be frozen; the card tests each of those. What is
 expected is a product decision: ask the developer rather than reading it off the code.
