...
| `write-manual-test`        | Write or rewrite a manual test case in Notion: exact steps, "Verify" checkboxes, a screenshot under every verification, and a prepared test book hosted on dev.bloomlibrary.org. |
...
| `interactive-dashboard`    | A live steering board for a long multi-agent run: every work item's state and log, drag-to-reorder priority, per-item pause, the workers' questions answered with a click, and a note box, all flowing back to the controlling agent through the page's database. |
...
- **Never commit a token or secret here** — this repo is public. Skills reference tokens only
  by environment variable name (`$YOUTRACK_BOT`, `REVIEWABLE_API_TOKEN`, …). The
